...
function renderTools(filteredTools = aiTools) {
    ...
    if (filteredTools.length === 0) {
        toolsGrid.innerHTML = '<p style="text-align: center; grid-column: 1/-1; color: var(--text-secondary);">No tools found matching your criteria.</p>';
        return;
    }
    ...
    filteredTools.forEach(tool => {
        ...
        toolCard.innerHTML = `
            <div class="tool-header">
                <img src="${logoUrl}" alt="${tool.name}" class="tool-logo" onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name=${encodeURIComponent(tool.name)}&background=random&color=fff'">
                <div class="tool-header-content">
                    <div class="tool-name">${tool.name}</div>
                    <div class="tool-category">${Array.isArray(tool.category) ? tool.category.map(getCategoryName).join(', ') : getCategoryName(tool.category)}</div>
                </div>
            </div>
...
function filterTools() {
    ...
    const ratingFilter = ratingFilterElement ? parseFloat(ratingFilterElement.value) || 0 : 0;
    ...
    const resultsHTML = recommendations.map(tool => {
        ...
        return `
            <div class="result-card" onclick="showToolDetails(${JSON.stringify(tool).replace(/"/g, '&quot;')})">
                <h3>${tool.name}</h3>
                <div class="tool-category" style="display: inline-block; margin: 0.5rem 0;">${Array.isArray(tool.category) ? tool.category.map(getCategoryName).join(', ') : getCategoryName(tool.category)}</div>
